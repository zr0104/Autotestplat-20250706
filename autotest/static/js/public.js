...
function loginVerify(){
            ...
            if(userObj == null){
                ...
                $.ajax({
                    ...
                    success: function(rst){
                        if(rst == 200){
                            ...
                            var testDropdownMenu = document.getElementById('testDropdownMenu');
                            var apitestDropdown = document.getElementById('apitestDropdown');
                ...
                $(function() {
                    ...
                    $(document).on('click', '.dropdown > a.nav-link', function(e) {
                        e.preventDefault();
...
function saveIterationOption() {
    var selectedOption = $("#select-option").val()
    var selectedIterationOption = $("#select-iteration-option").val();
    console.log(selectedOption)
    console.log(selectedIterationOption)
    if (selectedOption) {
        $.ajax({
            url: '/autotest/user/set/iteration/',
            type: 'POST',
            data: { 'product_name': selectedOption,'iteration_version': selectedIterationOption},
            success: function(response) {
                console.log('Saved successfully');
                location.reload();
                },
...
function retoaiCaseTableDataInit(){
    table = $('#table').DataTable({
        dom:'tp',
        pageLength: tableItemsPerPage,
        ajax: {
...
function showTestReport(e){
    selectedRow = e.parentNode.parentNode
    var inputFields = $("[name=reportInput]")
    for(let i = 0; i < inputFields.length; i++){
        var temp = inputFields[i]
        if(temp.tagName == "INPUT"){
            temp.value = selectedRow.children[reportRowIndex[i]].innerText
            if(temp.type == "password"){
                temp.value = $(selectedRow.children[reportRowIndex[i]]).find("span").attr("pdata")
            }
        }
        else if(temp.tagName == "SELECT"){
            $(temp).find("option[selected=true]").prop("selected",false)
            $(temp).find("option[value='"+ selectedRow.children[reportRowIndex[i]].innerText +"']").prop("selected",true)
        }
        else if(temp.tagName == "TEXTAREA"){
            temp.value = selectedRow.children[reportRowIndex[i]].innerText
        }
...
function showReToCaseModal(e){
    selectedRow = e.parentNode.parentNode
    var inputFields = $("[name=reToCaseInput]")
    console.log(inputFields)
    for(let i = 0; i < inputFields.length; i++){
        var temp = inputFields[i]
        if(temp.tagName == "INPUT"){
            temp.value = selectedRow.children[reToCaseRowIndex[i]].innerText
            console.log(temp.value)
            if(temp.type == "password"){
                temp.value = $(selectedRow.children[reToCaseRowIndex[i]]).find("span").attr("pdata")
            }
        }
        else if(temp.tagName == "SELECT"){
            $(temp).find("option[selected=true]").prop("selected",false)
            $(temp).find("option[value='"+ selectedRow.children[reToCaseRowIndex[i]].innerText +"']").prop("selected",true)
        }
        else if(temp.tagName == "TEXTAREA"){
            temp.value = selectedRow.children[reToCaseRowIndex[i]].innerText
        }
    }
    console.log($("#reToCaseModal"))
    $("#reToCaseModal").modal('show')
    window.event.stopPropagation()
}
...
function versionTestReport(tips="生成测试报告成功"){
    var inputFields = $("[name=reportInput]")
    for(var i = 0; i < reportRequiredFields.length; i++){
        if(inputFields[reportRequiredFields[i]].value == ''){
            console.log(inputFields[reportRequiredFields[i]].value)
            return alert("*信息为必填项！")
        }
    }
    $.ajax({
        url: appURL + reportURL,
        type: "POST",
        aysnc: false,
        data: AjaxObjectData(reportFieldNames,inputFields),
        success: (rst) => {
            if(rst === '200'){
                // operationSelectValue('modify')
                alert(tips)
...
function reTotestcase(tips = "生成用例成功,请到AI测试用例模块查看") {

    document.getElementById('tipsModal').style.zIndex = '9999';
    document.getElementById('tipsModal').style.display = 'block';

    var inputFields = $("[name=reToCaseInput]");
    // 验证必填字段
    for(var i = 0; i < reToCaseRequiredFields.length; i++) {
        if(inputFields[reToCaseRequiredFields[i]].value == '') {
    ...
    $.ajax({
        url: appURL + reToCaseURL,
        type: "POST",
        async: true, // 改为异步请求
        data: AjaxObjectData(reToCaseFieldNames, inputFields),
        beforeSend: function() {
            // 显示加载提示
            document.getElementById('tipsModal').style.display = 'block';
        },
        success: function(rst) {
            ...
            if(rst === '200') {
                alert(tips);
                $("#reToCaseModal").modal('hide');
                // 重置表单
                $("[name=reToCaseInput]").val('');
            } else {
                console.log(rst);
                alert('生成测试用例有误，请查看后台日志');
            }
        },
        error: function(rst) {
            document.getElementById('tipsModal').style.display = 'none';
